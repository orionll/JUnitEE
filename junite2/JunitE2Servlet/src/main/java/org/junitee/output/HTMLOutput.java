/**
 * $Id: HTMLOutput.java,v 1.1.1.1 2007-07-13 23:45:16 martinfr62 Exp $
 * $Source: C:\Users\Orionll\Desktop\junitee-cvs/junite2/JunitE2Servlet/src/main/java/org/junitee/output/HTMLOutput.java,v $
 */
    ...
    while (suites.hasNext()) {
      ...
      while (tests.hasNext()) {
        ...
        if (!test.successful()) {
          ...
          Failure t1 = test.getFailure();

          if (t1 != null) {
            String stackTrace = t1.getTrace();

            if (isFilterTrace()) {
              stackTrace = StringUtils.filterStack(stackTrace);
            }
            pw.println("<tr><td class=\"cell\">");
            pw.println(StringUtils.htmlText(t1.getMessage()));
            pw.println("&nbsp;</td>");

            pw.println("<tr><td class=\"cell\">");
            pw.println(StringUtils.htmlText(stackTrace));
            pw.println(StringUtils.htmlText(this.getEJBExceptionDetail(t1.getException())));
            pw.println("</td></tr>");
          }
          pw.println("<tr><td>&nbsp;</td></tr>");
        }
